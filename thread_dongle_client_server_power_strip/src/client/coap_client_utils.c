...
static bool coap_server_has_responded;
                     ...
                     const struct sockaddr *from)
{
     ...
     if (strstr(payload, cmd_ok) != NULL) {
          printk("THREAD CLIENT [DEBBUG]: commands msg reply: CMD_ACK\r\n"); 
                     ...
                     const struct sockaddr *from)
{
     ...
     coap_server_has_responded = true;

     printk("THREAD CLIENT [DEBBUG]: Power strip status reply received from server \r\n");     
     ...
     // Print payload
     printk("THREAD CLIENT [DEBBUG]: Received payload: size: %d  payload ", payload_size);
...
static void send_keep_alive(struct k_work *item)
{
     ...
     printk("THREAD CLIENT [DEBBUG]: Sending keep alive msg to server \r\n");
...
static void send_power_strip_status_request(struct k_work *item)
{
     ...
     printk("THREAD CLIENT [DEBBUG]: Sending power strip status request to server \r\n");

     coap_server_has_responded = false;
...
{         
     if (is_connected) {
          ...
          printk("THREAD CLIENT [ERROR]: Connection is broken \r\n");
          coap_server_has_responded = false;
     }
}
...
bool connected_to_coap_server()
{    
     if(!coap_server_has_responded){
          printk("THREAD CLIENT [INFO]: coap server not responding \r\n");
          return false;
     }
     return true;
}